...
Det ses let at i vores objektive funktion (\ref{eq:objective_function}), at for at opnaa den minimale vaerdie ville $w = 0$ vaere den bedste loesning, menne denne loesning ville ikke holde da vores betingelser i uligheden fra (\ref{eq:req_final}) ikke vil blive overholdt, og hvis uligheden ikke bliver overholdt, vil det ikke vaere muligt finde en loesning til problemet. For at loese dette problem, vil vi transformere vores objektive funktion \ref{eq:objective_function} til en \textit{Lagrangian} funktion, en \textit{Lagrangian} funktion inkorporerer vores ulighed fra (\ref{eq:req_final}) i vores objektive funktion (\ref{eq:objective_function}), og hvis uligheden ikke bliver overholdt, vil vaerdien af funktionen stige, og dermed give et daarlige resultat. Ved at have omskrevet vores objektive funktion (\ref{eq:objective_function}) til en \textit{Lagrangian} funktion, vil det ogsaa vaere muligt for os at bruge \textit{Lagrange multipliers}, og det ses ogsaa i funktionen (\ref{eq:L_P}), som er vores \textit{Lagragian} funktion hvor vores uligheder er blevet inkorpereret, samt at der er indsat et $\lambda$, som er vores \textit{Lagrange multiplier}.
\begin{equation}
\label{eq:L_P}
...
\section{Results: Working title}
\begin{table}[H]
\begin{center}
\scalebox{0.7} {
\begin{tabular}{c|c|c|c|c|c}
Method/Dataset & MUTAG & ENZYMES & DD & NCI1 & NCI109 \\
\hline
...
GraphHopper & $83.9444 (\pm 0.5705)$ & $36.2833 (\pm 0.4856)$ & - & $72.5985 (\pm 0.1323)$ & - \\
...
Shortest path & $85.6667 (\pm 0.7258)$ & - & - & $73.1946 (\pm 0.1146)$ & $73.1311 (\pm 0.0798)$ \\
\end{tabular}
}
